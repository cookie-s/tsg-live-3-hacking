...
def solve(ttf):
    ...
    goal = "T"
    print("glyph00346: '} is correct'")
    path = bfs(ttf, prev_glyph_map, start, goal)
...
def bfs(ttf, graph, start, goal):
    ...
    while queue:
        ...
        for (next_vertex, input_glyph) in edges:
            print('{}: {} -> {} : {} is correct'.format(vertex, next_vertex, vertex, glyph_names_to_chars(ttf, [input_glyph] + input_glyphs)))
            if next_vertex == goal:
